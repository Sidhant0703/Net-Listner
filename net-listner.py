...
def execute(cmd):
    ...
    if not cmd:
        return
    output = subprocess.check-output(shelx.split(cmd), stderr=subprocess.STDOUT)
...
class NetListner:
    ...
    def handle(self, client_socket):
        if self.args.execute:
            output=execuet(self.args.execute)
...
if __name__=='__main__':
    parser=argparse.ArgumentParser(description='Net Tool By Sidhant0703', formatter_class=argparse.RawDescriptionHelpFormatter, epilog=textwrap.dedent('''Example:
...
net-listner.py -t 192.8.7.11 -p 1234 # connect to server '''))
    ...
    nc= NetListner(args, buffer.encode())
    nc.run()
